...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    labelDate.textContent = new Intl.DateTimeFormat(currentAccount.locale, options).format(now); //pass local string

    /* const day = `${now.getDate()}`.padStart(2, 0); // 2 chars long
    const month = `${now.getMonth() + 1}`.padStart(2, 0); // 2 chars long
...
/* 
  178. Internationalizating Dates (intl)
*/